...
import { consultarClienteAction } from '@/app/(app)/cliente-360/actions';
...
export interface ConsultaClienteProps {
  inicial: ClienteRisco[];
}

export function ConsultaCliente({ inicial }: ConsultaClienteProps) {
  const [documento, setDocumento] = useState('');
  const [clientes, setClientes] = useState<ClienteRisco[]>(inicial);
  ...
  const consultar = () => {
    ...
    startTransition(async () => {
      try {
        const r = await consultarClienteAction(documento.trim());
        if (!r.ok || !r.cliente) {
          setErro(
            r.motivo === 'indisponivel'
              ? 'Consulta real requer contrato com Serasa / Receita Federal.'
              : 'Documento inválido. Informe um CPF (11 dígitos) ou CNPJ (14 dígitos).',
          );
          return;
        }
        const novo = r.cliente;
        // Substitui consulta anterior do mesmo documento, se houver, e põe no topo.
        setClientes((prev) => [novo, ...prev.filter((c) => c.documento !== novo.documento)]);
        setDocumento('');
      } catch (e) {
    ...
    <div className="space-y-4">
      <div className="nx-card flex flex-col gap-3 p-5 sm:flex-row sm:items-center">
        ...
        <button
          type="button"
          onClick={consultar}
          disabled={pending || !documento.trim()}
    ...
    <article className="nx-card flex flex-col gap-4 p-5">
      ...
      <div>
        ...
        <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-elevated">
          <div className={`h-full rounded-full ${nv.bar}`} style={{ width: `${cliente.score}%` }} />
        </div>
      </div>
